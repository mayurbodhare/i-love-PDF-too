...
const FeatureCard: React.FC<Feature> = ({
	...
	return (
		...
		>
			<div
				className={`h-16 w-16 ${bgColor} rounded-lg flex items-center justify-center mb-6`}
			>
				{icon}
			</div>
	...
	const features: Feature[] = [
		{
			...
			description:
				"Convert PDFs to and from multiple formats including Word, Excel, and PowerPoint",
			icon: (
				<svg
		...
		},
		{
			...
			title: "Merge & Split",
			description:
				"Combine multiple PDFs into one or split large documents into smaller parts",
			icon: (
				<svg
					className="h-8 w-8 text-white"
					fill="none"
		...
		},
		{
			...
			description:
				"Secure your sensitive documents with strong encryption and password protection",
			icon: (
				<svg
					className="h-8 w-8 text-white"
					fill="none"
		...
		},
		{
			...
			description:
				"Rearrange, rotate, and delete pages with intuitive drag-and-drop interface",
			icon: (
				<svg
					className="h-8 w-8 text-white"
					fill="none"
